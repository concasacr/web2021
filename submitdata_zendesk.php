...
if( $prt_macro == '' ){
	...
	$response["mensaje"] = "Tienes el campo del macro vacio y no podemos gerenar su reporte verifique e intente nuevamente ";
...
if( $prt_proyecto == '' ){
	$response["success"] = false;
	$response["mensaje"] = "Tienes el campo de proyecto vacio y no podemos gerenar su reporte verifique e intente nuevamente ";
	echo json_encode($response);
	exit();
}
...
if( $prt_phone == '' ){
	$response["success"] = false;
	$response["mensaje"] = "Tienes el campo de teléfono vacio y no podemos gerenar su reporte verifique e intente nuevamente ";
	echo json_encode($response);
	exit();
}
...
//$prt_nhome       = $_POST['tower'];
//$prt_nhome_02    = $_POST['floor']; 
//$prt_nhome_03    = $_POST['apartments'];
/*

if( $prt_nhome == '' ){
	$response["success"] = false;
	$response["mensaje"] = "Tienes campos vacios y no podemos gerenar su reporte verifique e intente nuevamente ";
	echo json_encode($response);
	exit();
}
if( $prt_nhome_02 == '' ){
...
if( $prt_macro == 'monte_alto' and $prt_proyecto == 'monte_alto'){
	
	$obj_conexion_PV = @mysqli_connect('localhost','concasac_UserPostVenta','!WT3Dr8ObW~8','concasac_ControlPostVenta');
	
 	$var_consulta_PV= "SELECT * FROM `proyects` WHERE `tbl_slug`= 'monte_alto' AND `tbl_tower` = '$prt_nhome' AND `tbl_flor` = '$prt_nhome_02' AND `tbl_apto` = '$prt_nhome_03'";
 	$var_resultado_PV = $obj_conexion_PV->query($var_consulta_PV);
    if($var_resultado_PV->num_rows>0){
	     $row = $var_resultado_PV->fetch_array();
		 $dateclose = $row['tbl_date_delivery'];
	}else{
		 $dateclose = '';
	}
}
...
if(!empty($_FILES["imagenticket"]["name"])){
	...
	//echo $_FILES['imagenticket']['type'];
	//var_dump($_FILES['imagenticket']);
	//die();
	
	$filename 		 = $_FILES['imagenticket']['name'];
	$extension 		 = pathinfo($filename, PATHINFO_EXTENSION);
	$attachment_type = 'image/'.$extension;
	$attachment      = $_FILES['imagenticket']['name'];
